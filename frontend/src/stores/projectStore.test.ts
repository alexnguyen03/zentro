...
describe('projectStore', () => {
    ...
    it('bootstraps projects and hydrates active project', async () => {
        const active = makeProject({ id: 'active-1', default_environment_key: 'dev' });
    ...
    it('updates environment and persists through saveProject flow', async () => {
        const current = makeProject({ id: 'p-env', default_environment_key: 'loc', environments: [] });
        ...
        const saved = makeProject({
            ...
            last_active_environment_key: 'dev',
            environments: [
                {
                    id: 'env-dev',
        ...
        const result = await useProjectStore.getState().setProjectEnvironment('dev');

        expect(projectServiceMock.saveProject).toHaveBeenCalledTimes(1);
        expect(result?.default_environment_key).toBe('dev');
        expect(result?.environments?.some((environment) => environment.key === 'dev')).toBe(true);
    });
});

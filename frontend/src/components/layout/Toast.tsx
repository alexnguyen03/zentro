...
export const ToastProvider: React.FC<ToastProviderProps> = ({ children, placement = 'bottom-left' }) => {
    const addToast = React.useCallback((message: string, variant: ToastVariant) => {
        ...
        sonnerToast(message, {
            ...
            className: cn(
                'relative pointer-events-auto flex min-w-[280px] max-w-[520px] items-start gap-2 !rounded-sm border border-border/70 bg-card px-6 py-5 pr-12 text-[15px] shadow-elevation-md',
            ),
        });
    }, []);
    ...
    return (
        <ToastContext.Provider value={{ toast }}>
            ...
            <Toaster
                ...
                toastOptions={{
                    classNames: {
                        title: 'flex-1 break-words pr-1 leading-[1.35] text-[15px] font-semibold text-foreground !rounded-sm',

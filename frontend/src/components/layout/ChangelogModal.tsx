...
export const ChangelogModal: React.FC<ChangelogModalProps> = ({ onClose }) => {
    return (
        ...
        >
            <div className="min-h-0 flex-1 overflow-y-auto p-6 text-small leading-relaxed markdown-body">
                <ReactMarkdown 
                    remarkPlugins={[remarkGfm]}
                    components={{
                        h1: ({node, ...props}) => <h1 className="text-h1  mt-2 mb-4 pb-2 border-b border-border" {...props} />,
                        h2: ({node, ...props}) => <h2 className="text-h2  mt-6 mb-3 text-accent" {...props} />,
                        h3: ({node, ...props}) => <h3 className="text-h3  mt-5 mb-2" {...props} />,
